...
class ColonyCounter:
    ...
    def draw_points(self):
        ...
        try:
            ...
            BRIGHT_GREEN = (102, 255, 102)  # 밝은 녹색 (BGR)
            MANUAL_OUTLINE_COLOR = (0, 0, 0)  # 검은색
            ...
            for idx, (x, y) in enumerate(self.manual_points, len(self.auto_points) + 1):
                ...
                # 반투명 사각형 (밝은 녹색)
                cv2.rectangle(overlay, pt1, pt2, BRIGHT_GREEN, -1)
                cv2.rectangle(overlay, pt1, pt2, MANUAL_OUTLINE_COLOR, 2)
                # 번호 텍스트 추가 (밝은 녹색, 외곽선 검정)
                text = str(idx)
                (text_width, text_height), _ = cv2.getTextSize(text, font, font_scale, font_thickness)
                text_x = int(display_x - text_width / 2)
                text_y = int(display_y - 10)
                ...
                cv2.putText(overlay, text,
                          (text_x, text_y),
                          font, font_scale, BRIGHT_GREEN, font_thickness)
            ...
            # 4. 좌측하단에 최종 카운트(스코어) 표시 (밝은 녹색, 외곽선 검정)
            ...
            score_text = f"Total: {total_count}  (Auto: {auto_count}, Manual: {manual_count})"
            font_score = cv2.FONT_HERSHEY_SIMPLEX
            font_scale_score = 1.2
            font_thickness_score = 3
            margin = 20
            text_size, _ = cv2.getTextSize(score_text, font_score, font_scale_score, font_thickness_score)
            # 배경 박스
            cv2.rectangle(
                img_with_points,
                (10, img_with_points.shape[0] - text_size[1] - margin),
                (10 + text_size[0] + margin, img_with_points.shape[0] - 10),
                (0, 0, 0),
                -1
            )
            # 8방향 검정 외곽선
            for dx, dy in [(-2,0),(2,0),(0,-2),(0,2),(-2,-2),(2,2),(-2,2),(2,-2)]:
                cv2.putText(
                    img_with_points, score_text,
                    (20 + dx, img_with_points.shape[0] - margin + dy),
                    font_score, font_scale_score, (0, 0, 0), font_thickness_score + 1, cv2.LINE_AA
                )
            # 밝은 녹색 텍스트
            cv2.putText(
                img_with_points, score_text,
                (20, img_with_points.shape[0] - margin),
                font_score, font_scale_score, BRIGHT_GREEN, font_thickness_score, cv2.LINE_AA
            )

            # === 타입/shape 보정 및 PIL 변환 추가 ===
            if not isinstance(img_with_points, np.ndarray):
                img_with_points = np.array(img_with_points)
            if img_with_points.ndim == 2:
                img_with_points = cv2.cvtColor(img_with_points, cv2.COLOR_GRAY2BGR)
            if img_with_points.dtype != np.uint8:
                img_with_points = img_with_points.astype(np.uint8)
            return Image.fromarray(img_with_points)
        except Exception as e:
...
class ImagePreprocessHistory:
    ...
    def set_original(self, image):
        ...
        # PIL Image로 변환 (dtype/shape/채널 보정 포함)
