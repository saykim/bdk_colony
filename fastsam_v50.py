...
class ImageManager:
    ...
    def start_new_session(self, image_name="unknown"):
        """새로운 분석 세션 시작"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        self.current_session_id = f"{timestamp}_{image_name}"
        return self.current_session_id

    def save_initial_results(self, original_image, analyzed_image, metadata):
        """초기 분석 결과 저장"""
        try:
            ...
            # 원본 이미지 저장
            original_path = self.base_path / "images" / "original" / f"{self.current_session_id}_original.jpg"
            Image.fromarray(original_image).save(original_path)
            
            # 분석된 이미지 저장
            analyzed_path = self.base_path / "images" / "analyzed" / f"{self.current_session_id}_initial.jpg"
            Image.fromarray(analyzed_image).save(analyzed_path)

            # 초기 메타데이터 저장
            metadata_path = self.base_path / "metadata" / f"{self.current_session_id}_metadata.json"
            metadata["created_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            
            with open(metadata_path, 'w', encoding='utf-8') as f:
                json.dump(metadata, f, indent=4, ensure_ascii=False)

            logger.info(f"Initial results saved for session: {self.current_session_id}")
        except Exception as e:
            logger.error(f"Error saving initial results: {str(e)}")
            raise

    def update_session(self, analyzed_image, metadata):
        """최종 결과 업데이트"""
        try:
            if self.current_session_id is None:
                raise ValueError("No active session")
            ...
            # 최종 분석 이미지 저장
            analyzed_path = self.base_path / "images" / "analyzed" / f"{self.current_session_id}_final.jpg"
            Image.fromarray(analyzed_image).save(analyzed_path)
            ...
            try:
                ...
                existing_metadata["last_updated"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            ...
            logger.info(f"Session updated: {self.current_session_id}")
            return True
        except Exception as e:
...
class ColonyCounter:
    ...
    def create_overlay(self, original_image, analyzed_image):
        ...
        try:
            ...
            # 이미지 크기 맞추기
            if original_image.shape != analyzed_image.shape:
                analyzed_image = cv2.resize(analyzed_image, 
    ...
    def draw_points(self):
        try:
            ...
            for idx, (x, y) in enumerate(self.manual_points, len(self.auto_points) + 1):
                ...
                cv2.putText(img_with_points, text,
                            (text_x, text_y),
                            font, font_scale, (255, 0, 0), font_thickness)  # (B, G, R) 형식으로 빨간색
    ...
    def save_final_results(self):
        ...
        try:
            ...
            if self.image_manager.current_session_id is None:
                # 세션이 없는 경우 새로 생성
                image_name = "unknown"
                self.image_manager.start_new_session(image_name)
            ...
            final_metadata = {
                ...
                "manual_points": [(int(x), int(y)) for x, y in self.manual_points],
                "completion_time": datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            }

            # 최종 이미지와 메타데이터 업데이트
            self.image_manager.update_session(
                self.current_image,
                final_metadata
            )

            return f"Results saved successfully! (Session ID: {self.image_manager.current_session_id})"
        except Exception as e:
...
def segment_and_count_colonies(
    ...
    try:
        ...
        # 새로운 세션 시작
        image_name = getattr(input_image, 'filename', 'unknown')
        counter.image_manager.start_new_session(image_name)
...
with gr.Blocks(theme=gr.themes.Soft(), css=css) as demo:
    ...
    with gr.Tab("콜로니 카운터"):
        ...
        with gr.Row(elem_classes="instruction-box"):
            gr.Markdown(
                ...
                5. **분석 설정 조정**:
                   - **입력 크기**: 입력 이미지 크기를 설정하세요 (크면 정확도 증가, 속 감소).
    ...
    with gr.Row():
        ...
        save_result_text = gr.Textbox(
            label="저장 결과",
            lines=1
        )

    # 이벤트 핸들러에 저장 버튼 이벤트 추가
    save_button.click(
        counter.save_final_results,
        inputs=[],
